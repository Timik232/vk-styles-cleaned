...
### Fixed
- В fix-режиме поднят слой `#layout_sidebar`/`#side_bar_inner` выше кнопки `pg_fixed_back`, чтобы кнопка «наверх» не блокировала клики по боковому меню

## [2.0.8.3] - 2026-04-13

### Fixed
- Добавлен верхний внутренний отступ fixed-меню в `html.mb #layout_sidebar [class*=LeftMenu]>nav`
- Убрано `overflow-y: auto` у `html.mf #layout_sidebar`, чтобы не создавать лишнюю прокрутку в fix-режиме
...
- Добавлена поддержка `vk.ru` и `*.vk.ru` в Chromium-манифесте
- Обновлены VK-селекторы в `css` и `mcss` для новых вариантов шапки, меню, поповеров, логотипа и видео-разметки

### Fixed
- Обновлены селекторы аудиостраницы для новых вариантов плеера VK, включая современный контейнер кнопок и варианты ползунка громкости
- Добавлены новые селекторы блоков рекомендаций и промо, чтобы снова скрывать лишние промо-вставки в обновленной верстке VK
- Улучшена интеграция навигации по фото/темам в панели настроек
- Улучшена загрузка видеоальбомов для live wallpaper: получение альбомов, сортировка, фильтрация пустых альбомов и безопасный fallback
- Исправлена функция `Lg()`: теперь она выбирает самый новый подходящий аудиоключ по `_vNNN`, а не первое совпадение
- Исправлен false positive в GitHub Actions, из-за которого корректное использование `atob()` внутри существующего `atou()` ошибочно считалось запрещенным
- Исправлена боковая панель: восстановлен внешний вид из `vk styles 06 версия`, убраны наложения, возвращено корректное позиционирование в mini+fix и сохранена корректная обрезка активного пункта
- Добавлены корректные отступы fixed-боковой панели (включая исключение для мини-режима)
- Увеличен верхний внутренний отступ fixed-боковой панели, чтобы верхний пункт меню не выступал за границы
- Устранена обрезка слева в режиме «Фикс»: боковая панель снова визуально совпадает с обычным режимом
- Перенастроены внутренние отступы fixed-меню (увеличены сверху и слева), чтобы убрать выступ сверху и обрезку слева
- Для fixed-режима удалены пост-v06 принудительные overflow/padding-правила у LeftMenu nav, возвращено поведение как в версии 06
- Исправлены стробы фона: восстановлена формула яркости и увеличена интенсивность белых вспышек
